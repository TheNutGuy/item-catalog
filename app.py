...
engine = create_engine(
    'sqlite:///catalogproject.db', connect_args={'check_same_thread': False}
    )
...
def delete_category(category_id):
    ...
    if not exists_category(category_id):
        flash("We can't process your request right now.")
        return redirect(url_for('home'))
...
def add_item_by_category(category_id):
    if 'username' not in login_session:
        ...
        category = session.query(Category).filter_by(id=category_id).first()
        return render_template('NewItem2.html', category=category)
...
def catalog_item_json(category_id, item_id):
    if exists_category(category_id) and item_dupe(item_id):
        ...
        if item is not None:
            ...
            return jsonify(
                error='item {} does not belong to that category {}.'
